...
public class PHormiga extends JPanel implements ActionListener {
    private transient FactoryBL<HormigaDTO> blFactory = new FactoryBL<>(HormigaDAO.class);
    private transient HormigaDTO dtoHormiga = new HormigaDTO();
    ...
    private void loadRowData() throws Exception {
        regAct = blFactory.getMinReg("idHormiga");
        regMax = blFactory.getMaxReg("idHormiga");
        dtoHormiga = blFactory.getBy(regAct);
    }

    private void showRowData() {
        boolean isDTONull = (dtoHormiga == null || dtoHormiga.getIdSexo() == null);
        txtIdSexo.setText((isDTONull) ? " " : dtoHormiga.getIdSexo().toString());
        txtNombre.setText((isDTONull) ? " " : dtoHormiga.getNombre());
    ...
    private void showDataTable() throws Exception {
        ...
        int index = 0;
        for (HormigaDTO d : blFactory.getAll()) {
    ...
    private void btnGuardarClick() {
        boolean isDTONull = (dtoHormiga == null);
        try {
            if (AppMSG.showConfirmYesNo("¿Seguro que desea " + ((isDTONull) ? "AGREGAR ?" : "ACTUALIZAR ?"))) {
                if (isDTONull)
                    dtoHormiga = new HormigaDTO(0, 1, 1, 1, txtNombre.getText().trim(), "");
                else
                    dtoHormiga.setNombre(txtNombre.getText());

                if (!((isDTONull) ? blFactory.add(dtoHormiga)
                        : blFactory.upd(dtoHormiga)))
    ...
    private void btnEliminarClick() {
        try {
            if (AppMSG.showConfirmYesNo("Seguro que desea Eliminar?")) {
                ...
                if (!blFactory.del(dtoHormiga.getIdSexo()))
    ...
    private PatLabel lblTitulo = new PatLabel("TIPO DE SEXO"),
            lblIdSexo = new PatLabel(" Código Sexo :"),
            lblNombre = new PatLabel("*Tipo de Sexo:"),
            lblTotalReg = new PatLabel(" 0 de 0 ");
    private PatTextBox txtIdSexo = new PatTextBox(),
